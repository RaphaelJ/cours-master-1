...
# Permits the access to Web, PDNS, RSYNC and SMTP/IMAP from the Internet.
...
iptables -A FORWARD -i eth4 -p tcp -s 192.168.3.10 -o eth2 -d 192.168.1.12 -j ACCEPT
iptables -A FORWARD -i eth4 -p udp -s 192.168.3.10 -o eth2 -d 192.168.1.12 -j ACCEPT
iptables -A FORWARD -i eth4 -p tcp -s 192.168.3.11 -o eth2 -d 192.168.1.12 -j ACCEPT
iptables -A FORWARD -i eth4 -p udp -s 192.168.3.11 -o eth2 -d 192.168.1.12 -j ACCEPT
